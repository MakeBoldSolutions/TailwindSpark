...
interface UseVariantsReturn {
  variants: AIVariant[];
  categories: AIVariantCategory[];
  loading: boolean;
  error: string | null;
  filterByCategory: (category: AIVariantCategory | null) => void;
...
export function useVariants(): UseVariantsReturn {
  ...
  const [categoryFilter, setCategoryFilter] = useState<AIVariantCategory | null>(null);
  ...
  const categories = useMemo(
    () => [...new Set(variants.map(v => v.category))] as AIVariantCategory[],
    [variants],
  );
  ...
  const filterByCategory = useCallback((cat: AIVariantCategory | null) => setCategoryFilter(cat), []);
