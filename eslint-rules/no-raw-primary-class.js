...
export const rules = {
  'no-raw-primary-class': {
    meta: {
      ...
      messages: {
        noPrimary:
          'Avoid raw "{{cls}}". Use semantic brand / surface / text / data-viz tokens instead.',
      },
      schema: [],
    },
    create(context) {
      ...
      const prefixes =
        'bg|text|border|from|to|via|ring|outline|decoration|divide|caret|accent|shadow';

      // Optional modifiers (hover, focus, dark, etc.)
      const modifiers =
        '(?:(?:hover|focus|active|disabled|focus-visible|dark|group-hover|peer-hover):)*';
