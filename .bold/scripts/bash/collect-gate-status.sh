...
printf '{"active_features":%s,"backbone_principles":%s,"test_config_present":%s,"has_uncommitted_changes":%s}\n' \
  "$active_features" "$backbone_principles" "$(json_array "${test_config_present[@]}")" "$has_uncommitted_changes"
