...
# tier would serve it — user (.bold-user/) > team (bold-docs/overrides/) >
# source (.bold/) — first hit wins, whole-file replacement only (no
# fragment merging). Doubles as the `bold which` diagnostic: it reports the
# full chain, not just the winner.
param(
...
$userPath = Join-Path $repoRoot ".bold-user/$RelPath"
